...
- **电脑物理键盘直输**——电脑浏览器打开同一个 Handmux URL，无需点「展开键盘」即可直接输入当前 tmux pane。右侧蓝点 + 灰字 `键盘直通终端` 表示物理键盘归终端；点击原有的完整聊天输入框后，快捷用语、上传、历史、语音、草稿和多行自动增高全部保留，`Enter` 发送、`Shift+Enter` 换行，输入法合成中的 Enter 只确认候选词。终端回显仍跟随 Handmux 的快照刷新，并非实时 PTY；`Cmd+W/T/L/R` 等浏览器保留组合不承诺发送到终端。
...
- **内置浏览器 + 静态预览**——顶栏浏览器入口紧邻“文件”左侧：存在直连页签时右上角显示蓝点，任一页签开启代理时优先显示黄点，图标本身不变色。最左侧无文字 Home 标签打开“最近访问”，同一个标签会话内只保留最后跳转的网址；新建或切换页签时标签栏会自动滚动到当前页签。新页面默认手机直连，但可在输入网址前通过地址栏“…”卡片的“连接方式”分段按钮选择直连或电脑代理，地址栏标签会即时同步，打开后仍可切换当前页签；在电脑运行 `handmux setup`、选择“内置浏览器”并配置预览域名（同时为通配子域名配好路由与证书）后，代理选项才可用，未配置时界面会保持禁用并给出这条引导。该卡片还统一提供手机 / 电脑视图和各页签独立的 10 / 30 / 60 / 120 分钟后台关闭。Home 菜单用一个默认关闭的开关控制是否将代理 Cookie 加密持久化到电脑，以便重启后保持登录状态，并提供全部清理及可随时重看安全告知的“关于内置浏览器”；代理网页菜单只提供本站清理，清理 Cookie 不会关闭页签。相应问号与操作同一行；代理网页加载条为橙色，直连仍为蓝色。标签页、顺序、当前页、最近访问和计时全部保存在当前设备；代理 worker 只持有可按原始网址重建的运行租约。每个代理标签使用独立 Hammerhead Session，同一设备只共享按真实目标 Cookie 规则匹配的 Cookie Profile，因此企业 SSO 可以跨系统复用，但不会与其他设备或直连模式共享标签、localStorage、IndexedDB 或 Service Worker。切换标签保留已挂载页面，刷新会真正重新导航。直连仍受跨域 iframe 限制（`X-Frame-Options`、CSP、第三方 Cookie、标题与页面内跳转不可读）；代理支持 WebSocket，但因额外转发和页面改写通常更慢，特殊安全策略或浏览器能力仍可能不兼容，内置浏览器不保证所有网站都能正常使用。系统不会静默切换模式；显式允许的本机网址也只能访问相同 scheme、host 和 port。纯目录静态站仍用静态预览。
