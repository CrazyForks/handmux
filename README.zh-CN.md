...
- **内置浏览器 + 静态预览**——新页签默认由手机直连；配置 `previewDomain`（并为 `*.<previewDomain>` 配好路由与证书）后，也可选择经电脑代理，每个页签都能独立切换。同一设备的代理页签共用一个 Cookie Profile，仍按真实目标的 Domain、Path、Secure 等 Cookie 规则匹配，因此可在企业系统间复用 SSO 登录；Profile 永不与其他设备共享，直连模式的 Cookie 与页面状态仍由手机浏览器独立管理。显式打开目标 Origin 时使用固定短代理域名 `b-<13 位小写 base36>.<previewDomain>`；进入页面后的链接、表单和重定向留在当前 Hammerhead 会话中，保留原生 method、body 与跳转语义。工具栏暂不展示不可靠的前进 / 后退按钮，可从设备本地历史重新打开之前的网址。每台设备可单独选择是否在主机上加密持久化，默认关闭；当该设备没有代理页签后，可按 1 天、7 天、30 天（默认）或永不自动清理。可清理单个网站匹配的 Cookie 与代理页签，也可清理该设备全部代理登录状态；清理父域 Cookie 可能让同一父域下的相关系统同时退出。以上清理不影响直连页签、其他设备或历史。设备本地历史直接显示每个网址最后使用的直连 / 代理模式，也可单独删除一条记录。跨系统复用仅限 Cookie，不包括 localStorage、IndexedDB 或 Service Worker。直连仍受跨域 iframe 限制（`X-Frame-Options`、CSP、第三方 Cookie、标题与页面内跳转不可读），系统也不会在两种路径间静默回退。手机 / 电脑视口、各页签独立的 10 / 30 / 60 / 120 分钟或永不关闭计时照常可用；纯目录静态站仍用静态预览。
