...
- **🧶 你真实的会话,装进口袋。** 不是新开个 shell、也不是截图——是你电脑上**那一个** tmux pane,agent
  照跑。工位 → 手机 → 工位,全程同一个会话。
- **🤖 为「跟 agent 一起 vibe 编程」而生。** Claude Code 和 Codex 都是一等公民:要你时立刻推送、收件箱
  标好哪个 pane「进行中 / 等你 / 已完成」、各 agent 用量条、批计划批授权点一下就过。aider 及任意
  shell/TUI 也都行。
...
Agent 收件箱和「需要你就推给你」靠的是 agent 的生命周期 hook——Claude Code 和 Codex 都已接好。它
**默认不开、需你显式开启**——`handmux hooks install` 会把一个很小的通知脚本拷进 `~/.claude/hooks/`,
往 `~/.claude/settings.json` 注册六个 hook 事件,用 Codex 的话还会往 `~/.codex/config.toml` 追加一段
带标记的 hooks 区域(幂等;不动你自己的 hook)。`handmux setup` 也会问一句,首次在手机上打开收件箱时
还能一键开启。`handmux hooks uninstall` 全部移除。没装的 agent 直接跳过——绝不新建 `~/.claude` 或
`~/.codex`。
