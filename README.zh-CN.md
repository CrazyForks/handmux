...
- **电脑物理键盘直输**——电脑浏览器打开同一个 Handmux URL，无需点「展开键盘」即可直接输入当前 tmux pane。右侧蓝点 + 灰字 `键盘直通终端` 表示物理键盘归终端；点击「草稿区」后可编辑文字，快捷用语、上传、历史、语音和多行自动增高全部保留，`Enter` 发送、`Shift+Enter` 换行，输入法合成中的 Enter 只确认候选词；点击终端即可回到直通模式。终端回显仍跟随 Handmux 的快照刷新，并非实时 PTY；`Cmd+W/T/L/R` 等浏览器保留组合不承诺发送到终端。
