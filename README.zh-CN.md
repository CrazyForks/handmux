...
- **内置浏览器 + 静态预览**——顶栏浏览器入口紧邻“文件”左侧，新页签默认由手机直连；配置 `previewDomain`（并为 `*.<previewDomain>` 配好路由与证书）后，也可选择经电脑代理，每个页签都能独立切换。标签页、顺序、当前页、历史和后台关闭计时全部保存在当前设备；直连页面不经过 Handmux 服务端，代理 worker 只持有可按原始网址重建的运行租约，因此 worker 或主服务重启不会删除设备上的标签。每个代理标签使用独立 Hammerhead Session，同一设备只共享一个仍按真实目标 Domain、Path、Secure 等规则匹配的 Cookie Profile，所以企业 SSO 可以跨系统复用，但关闭一个标签不会带掉同站点的其他标签；Profile 永不与其他设备共享，直连模式的 Cookie 与页面状态仍由手机浏览器独立管理。显式打开目标 Origin 时使用固定短代理域名 `b-<13 位小写 base36>.<previewDomain>`；进入页面后的链接、表单和重定向留在当前 Session 中。切换标签只切换已挂载 iframe，不访问服务端也不重新加载；只有点击刷新才重载。工具栏不展示不可靠的前进 / 后退按钮，可从设备本地历史重新打开网址。每台设备可单独选择是否在主机上加密持久化 Cookie Profile，默认关闭，并可设置 1 天、7 天、30 天（默认）或永不自动清理；也可清理单站点或当前设备全部代理登录状态。以上清理不影响直连标签、其他设备或历史。跨系统复用仅限 Cookie，不包括 localStorage、IndexedDB 或 Service Worker。直连仍受跨域 iframe 限制（`X-Frame-Options`、CSP、第三方 Cookie、标题与页面内跳转不可读），系统不会在两种路径间静默回退。显式允许的本机网址只可访问相同 scheme、host 和 port，不能横向探测其他本机端口。手机 / 电脑视口、各标签独立的 10 / 30 / 60 / 120 分钟或永不关闭计时照常可用；纯目录静态站仍用静态预览。
