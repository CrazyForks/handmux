...
- **内置浏览器 + 静态预览**——顶栏浏览器入口紧邻“文件”左侧：存在直连页签时右上角显示蓝点，任一页签开启代理时优先显示黄点，图标本身不变色。最左侧无文字 Home 标签打开“最近访问”，同一个标签会话内只保留最后跳转的网址。新页面默认手机直连，但可在输入网址前通过地址栏“…”卡片的“连接方式”分段按钮选择直连或电脑代理，地址栏标签会即时同步，打开后仍可切换当前页签；配置 `previewDomain`（并为 `*.<previewDomain>` 配好路由与证书）后代理选项才可用。该卡片还统一提供手机 / 电脑视图和各页签独立的 10 / 30 / 60 / 120 分钟后台关闭。Home 菜单用一个“代理登录保留”下拉提供不保存（默认）、1 / 7 / 30 天或当前设备长期加密保存，并提供全部清理及可随时重看安全告知的“关于内置浏览器”；代理网页菜单只提供本站清理。相应问号与操作同一行，用于说明作用范围和父域影响；代理网页加载条为橙色，直连仍为蓝色。标签页、顺序、当前页、最近访问和计时全部保存在当前设备；代理 worker 只持有可按原始网址重建的运行租约。每个代理标签使用独立 Hammerhead Session，同一设备只共享按真实目标 Cookie 规则匹配的 Cookie Profile，因此企业 SSO 可以跨系统复用，但不会与其他设备或直连模式共享标签、localStorage、IndexedDB 或 Service Worker。切换标签保留已挂载页面，刷新会真正重新导航。直连仍受跨域 iframe 限制（`X-Frame-Options`、CSP、第三方 Cookie、标题与页面内跳转不可读），系统不会静默切换模式；显式允许的本机网址也只能访问相同 scheme、host 和 port。纯目录静态站仍用静态预览。
