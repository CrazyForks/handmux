...
- **内置浏览器 + 静态预览**——新页签默认由手机直连；配置 `previewDomain`（并为 `*.<previewDomain>` 配好路由与证书）后，也可选择经电脑代理，每个页签都能独立切换。切换会重新加载 Handmux 最后已知的网址；直连与代理的 Cookie、登录态互不共享。历史仅存在当前设备，并以 `lastMode` 记住每个网址最后使用的模式；旧历史则采用当前默认方式。直连使用跨域 iframe，目标站可能被 `X-Frame-Options` / CSP 拒绝嵌入，第三方 Cookie 可能受限，Handmux 通常也无法读取标题或页面内跳转。系统不会在两种路径之间静默回退：代理不可用或直连迟迟打不开时，会请你手动选择。手机 / 电脑视口、各页签独立的 10 / 30 / 60 / 120 分钟或永不关闭计时照常可用；纯目录静态站仍用静态预览。
