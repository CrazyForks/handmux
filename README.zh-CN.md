...
- **更新后重新加载客户端**——服务端更新完成后，在设置首层点「重新加载应用」即可载入新版客户端，无需退出主屏应用再重新打开。
...
**设备 key** 在手机 App 的设置 → 脚本推送中查看。它是寻址标识符,不是密钥——安全边界是本地服务器 token。同一 App 安装中关闭再开启推送会保持原 key，定向推送脚本无需跟着修改。
