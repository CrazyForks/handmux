...
import {
  fitRows, historyFillRows, bottomPadRows, scrollDecision, cursorBufferLine, followTarget,
...
import { openTerminalHistoryPreview } from '../terminalHistoryPreview.js';
...
const Terminal = forwardRef(function Terminal({
  ...
  useEffect(() => {
    ...
    historyHost.className = 'terminal__history';
    ...
    stackHost.className = 'terminal__stack';
    stackHost.append(historyHost, liveHost);
    elRef.current.replaceChildren(stackHost);
    ...
    const drawHistoryBoundary = () => {
      ...
      if (!historyMode || altScreenRef.current || !Number.isFinite(liveBoundaryLine)
    ...
    const clearStreamLayout = () => {
      ...
      historyPreview?.hide();
      elRef.current?.classList.remove('terminal--stream-exact', 'terminal--stream-composite');
      elRef.current?.style.removeProperty('--stream-grid-h');
      if (elRef.current) elRef.current.scrollTop = 0;
      liveHost.style.height = '100%';
    };
    const paintStreamPreview = () => {
      previewRaf = null;
      const config = previewConfig;
      if (disposed || !streamExact || !config || altScreenRef.current || config.rows < 1) {
        historyPreview?.hide();
        elRef.current?.classList.remove('terminal--stream-composite');
        return;
      }
      const rows = Math.min(config.rows, buf().baseY);
      if (rows < 1) {
        historyPreview?.hide();
        elRef.current?.classList.remove('terminal--stream-composite');
        return;
      }
      if (!historyPreview || !serializer) return;
      historyPreview.show();
      elRef.current?.classList.add('terminal--stream-composite');
      historyPreview.render({
        ...
        height: rows * config.cellH,
        onLayout: () => {
          if (!disposed && streamExact && !historyMode && elRef.current) {
    ...
    const scheduleStreamPreview = () => {
      if (previewRaf == null) previewRaf = requestAnimationFrame(paintStreamPreview);
    };
    if (streamMode) {
      import('@xterm/addon-serialize').then(({ SerializeAddon }) => {
        ...
        historyPreview = openTerminalHistoryPreview(historyHost, {
          fontSize: fontRef.current ?? 14,
          scrollback: MAX_LINES + 100,
        });
        scheduleFit();
      }).catch(() => {
        // The live terminal remains usable without the optional history fill.
      });
    }
    ...
    const enterStreamHistory = (pixels = 0) => {
      if (!streamMode || !streamExact || altScreenRef.current || buf().baseY < 1) return false;
      // Preserve the live parser-owned cursor before leaving the exact grid; history mode may resize
      // rows, after which the old seed cursor is no longer a trustworthy fallback.
      curInfo = {
        row: Math.max(0, paneRows - 1 - buf().cursorY),
        col: buf().cursorX,
    ...
    const fit = (pass = 0) => {
      ...
      // The preview is a second xterm and can report slightly different intrinsic metrics. Never let it
      // widen the shared stack: the raw-stream grid is authoritative for both layers.
      stackHost.style.width = `${Math.max(elRef.current.clientWidth, Math.ceil(screenRect.width))}px`;
      ...
      if (streamExact && paneRows) {
        ...
        const gridHeight = paneRows * cellH;
        const visibleRows = fitRows(avail, cellH);
        previewConfig = {
          rows: historyFillRows(
            visibleRows,
            paneRows,
            buf().baseY,
        ...
        elRef.current.classList.add('terminal--stream-exact');
        elRef.current.style.setProperty('--stream-grid-h', `${gridHeight}px`);
        liveHost.style.height = `${gridHeight}px`;
    ...
    const followLiveCursor = () => {
      ...
      const rowTop = liveHost.offsetTop + term.buffer.active.cursorY * rowHeight;
    ...
    const handleBufferScroll = () => {
      ...
      if (historyMode && historyResumeArmed && nearBottom()) {
        resumeStream();
        return;
      }
      if (streamMode && !nearBottom() && !altScreenRef.current) pauseStreamForHistory();
      if (historyMode && !nearBottom()) historyResumeArmed = true;
      setPaused(true);
      showScrollPos();
      maybePullMore();
    };
