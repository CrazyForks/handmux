...
export default function PreviewSheet({ open, name, kind = 'static', domain = null, port, dir, expiresAt, initialPath = '/', onRenew, onStop, onMinimize }) {
  ...
  const remainMs = useRemaining(expiresAt, open);
  // What to show after the name: a dynamic preview's port, a static preview's source dir.
  const detail = kind === 'dynamic' ? (port != null ? `:${port}` : '') : (dir || '');
    ...
    <div className={`file-sheet ${open ? 'open' : ''}`} aria-hidden={!open}>
      <div className="file-tabs preview-head">
        <span className="preview-status">
          <MonitorIcon />
          <span className="preview-state">{kind === 'dynamic' ? t('preview.dynamic') : t('preview.static')}</span>
          {name && (
            <span className="preview-name" title={`${name}${detail ? '  ' + detail : ''}`}>
              {name}{detail && <span className="preview-detail">{detail}</span>}
            </span>
          )}
        </span>
