...
export function openTerminalHistoryPreview(host, { fontSize, scrollback }) {
  ...
  const flush = async () => {
    ...
    while (pending && !disposed) {
      ...
      if (!disposed) {
        // This xterm can resolve font/cell metrics a fraction differently from the live WebGL xterm.
        // Size to its own complete rows after rendering; retaining the live estimate clips the boundary
        // row and makes the dashed history/live seam appear to cut through text.
        const renderedHeight = host.querySelector('.xterm-screen')?.getBoundingClientRect().height || 0;
        if (renderedHeight > 0) host.style.height = `${Math.ceil(renderedHeight)}px`;
        term.scrollToTop();
        frame.onLayout?.();
      }
    }
