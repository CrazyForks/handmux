...
export const CHANGELOG = [
  {
    ...
    items: {
      zh: [
        '新增「对话镜头」(实验性功能,默认关):到设置里打开「启用对话镜头(实验性功能)」后,窗口栏就能把 Claude 会话在终端/对话两个镜头间一键切换。对话镜头把会话读成聊天记录——气泡 + Markdown、工具卡(编辑文件带 +A/−B 和彩色 diff)、Claude 提问时点按即答的问题卡、上下文压缩动画、三套暖色配色;终端镜头照旧,完全不受影响。',
      ...
      en: [
        'New chat lens (experimental, off by default): enable it in Settings → Enable chat lens, then flip a Claude pane between terminal and chat from the window bar. The chat lens reads the session as a conversation — bubbles with Markdown, tool cards (+A/−B and colored diffs for file edits), question cards you answer with a tap, a visible compaction animation, and three warm colour tones. The terminal lens is untouched.',
