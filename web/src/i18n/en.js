...
export default {
  ...
  'scriptPush.key_hint': 'To push to THIS device only, add: --device <key>',
  'scriptPush.field_args': 'title and body are required (the two quoted arguments).',
  'scriptPush.field_session': '--session <name> — devices bound to that session (repeatable; default: all).',
  'scriptPush.field_device': '--device <key> — specific devices (repeatable). --session and --device are mutually exclusive.',
  'scriptPush.field_tag': '--tag <tag> — optional; the same tag replaces the previous push.',
  'scriptPush.field_url': '--url <path> — optional; opens this path when tapped.',
  'scriptPush.enable_first': 'Turn on push notifications above first — otherwise there is no device to push to (and no key to show).',
