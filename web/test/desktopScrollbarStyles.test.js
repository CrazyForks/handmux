...
describe('desktop scrollbar styles', () => {
  ...
  it('uses the measured live-grid width for both the live grid and its history preview', () => {
    expect(styles).toMatch(/\.terminal__stack\s*\{[^}]*width:\s*100%[^}]*min-width:\s*100%/);
    expect(styles).toMatch(
      /\.terminal--stream-exact \.terminal__stack\s*\{[^}]*align-items:\s*stretch/,
    );
    expect(terminalSource).toMatch(
      /stackHost\.style\.width\s*=\s*`\$\{Math\.max\(elRef\.current\.clientWidth,\s*Math\.ceil\(screenRect\.width\)\)\}px`/,
    );
    expect(styles).toMatch(
      /\.terminal--stream-composite \.terminal__live::before\s*\{[^}]*top:\s*-1px/,
    );
  });
