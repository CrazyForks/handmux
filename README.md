...
- **Built-in browser + static preview** — the Browser shortcut sits immediately left of Files in the top bar, and new tabs default to opening directly on the phone. Once `previewDomain` is configured (with routing and a certificate for `*.<previewDomain>`), any tab can instead use the computer proxy. Tabs, order, selection, history, and background-close timers live entirely on the current device; direct pages never create server-side tabs, while the proxy worker owns only disposable runtime leases that can be rebuilt from each saved canonical URL. A worker or main-service restart therefore does not delete device tabs. Every proxy tab has its own Hammerhead Session, while tabs on one device share only a Cookie Profile matched against each real target's Domain, Path, Secure, and other Cookie rules. Enterprise SSO can carry across systems without coupling sibling tabs, other devices, or direct-mode browser state. Explicit target origins use a stable short host, `b-<13 lowercase base36>.<previewDomain>`. Switching tabs only reveals an already-mounted iframe—there is no server synchronization or reload; only Refresh reloads. Back/Forward controls remain hidden because their state is unreliable, but device-local history can reopen a URL. Per-device encrypted Cookie Profile persistence defaults off and can expire after 1, 7, or 30 days (default), or never; one site or the current device's entire proxy login state can be cleared without affecting direct tabs, other devices, or history. Only Cookies are shared across proxied systems—not localStorage, IndexedDB, or Service Workers. Direct mode still has cross-origin iframe limits (`X-Frame-Options`, CSP, third-party-cookie policy, unreadable titles/navigation), and Handmux never silently changes modes. An explicitly allowed loopback target is restricted to the exact scheme, host, and port rather than gaining access to other local ports. Phone/desktop viewports and independent 10/30/60/120-minute or never-close tab timers remain available; folder preview still serves static sites.
