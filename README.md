...
- **Built-in browser + static preview** — the Browser shortcut sits immediately left of Files in the top bar. Its icon turns blue when direct tabs are open and orange when at least one proxy tab is open. The address bar’s **…** card keeps webpage zoom controls available for both modes, stepping through 75 / 80 / 90 / 100 / 110 / 125 / 150 / 175 / 200%; the webpage stays interactive, tabs and the address bar stay fixed, and no control floats over page content. Enlarging preserves the page’s existing layout width and exposes scrollbars for overflow instead of triggering a mobile reflow. The icon-only Home tab opens Recently Visited; one tab session keeps only its latest navigated URL, and opening or selecting a tab automatically scrolls it into view. New pages default to direct mode, but the address bar’s **…** card can select direct or computer proxy before opening the URL; the address badge updates immediately, and an existing tab can still switch afterward. Run `handmux setup` on the computer, choose **Built-in browser**, and configure its preview domain (plus wildcard routing and TLS) to enable proxy mode; before configuration, the disabled choice shows this guidance. That card also contains phone/desktop view and independent 10/30/60/120-minute background-close timers. On Home, an off-by-default switch controls whether proxy Cookies are encrypted and persisted on the computer so sign-ins survive a restart; clear-all and About sit beside it. A proxy page offers clear-this-site only; clearing proxy Cookies never closes its tabs. Contextual **?** buttons explain each action. Proxy pages use an orange loading bar while direct pages remain blue. If loading does not finish within five seconds, an actionable message explains likely direct/proxy causes and offers wait, retry, or an explicit mode switch. Tabs, order, selection, recent visits, and timers live entirely on the current device, while the proxy worker owns only disposable runtime leases rebuilt from canonical URLs. Every proxy tab has its own Hammerhead Session, and tabs on one device share only standards-matched Cookies, allowing enterprise SSO without sharing localStorage, IndexedDB, Service Workers, tabs, or state with other devices or direct mode. Switching tabs preserves the mounted iframe; Refresh performs a real navigation. Direct mode still has cross-origin iframe limits (`X-Frame-Options`, CSP, third-party-cookie policy, unreadable titles/navigation). Proxy mode supports WebSocket, but extra forwarding and page rewriting usually make it slower, while specialized security policies or browser capabilities may remain incompatible; the built-in browser cannot guarantee every site will work. Handmux never silently changes modes. Explicitly allowed loopback targets remain restricted to the exact scheme, host, and port. Folder preview still serves static sites.
