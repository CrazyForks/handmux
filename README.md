...
- **Built-in browser + static preview** — the Browser shortcut sits immediately left of Files in the top bar. Its bottom-right dot is blue when direct tabs are open and yellow when at least one proxy tab is open. New URLs always start in direct mode. Once `previewDomain` is configured (with routing and a certificate for `*.<previewDomain>`), each tab can opt into the computer proxy from the address bar’s **…** card using a large text segmented control. That card also contains phone/desktop view, independent 10/30/60/120-minute background-close timers, per-device encrypted Cookie Profile persistence (off by default), 1/7/30-day or never-expiring login retention, login cleanup, and the browser off switch; these controls are no longer duplicated in Settings. Tabs, order, selection, history, and timers live entirely on the current device, while the proxy worker owns only disposable runtime leases rebuilt from canonical URLs. Every proxy tab has its own Hammerhead Session, and tabs on one device share only standards-matched Cookies, allowing enterprise SSO without sharing localStorage, IndexedDB, Service Workers, tabs, or state with other devices or direct mode. Switching tabs preserves the mounted iframe; Refresh performs a real navigation. Direct mode still has cross-origin iframe limits (`X-Frame-Options`, CSP, third-party-cookie policy, unreadable titles/navigation), and Handmux never silently changes modes. Explicitly allowed loopback targets remain restricted to the exact scheme, host, and port. Folder preview still serves static sites.
