...
- **Desktop physical keyboard input** — open the same Handmux URL on a desktop browser and type into the current tmux pane immediately, without tapping “Show keyboard”. A blue dot with `Keyboard to terminal` means the terminal owns the physical keyboard; click the existing full chat composer to keep a draft, use shortcuts, upload, history or voice, then press `Enter` to send (`Shift+Enter` inserts a newline; IME Enter only confirms composition). Echo still follows Handmux’s terminal snapshots rather than a real-time PTY. Browser-reserved shortcuts such as `Cmd+W/T/L/R` stay with the browser.
