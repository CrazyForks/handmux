...
- **Real-time terminal + desktop physical keyboard input** — terminal output streams as it happens on both phone and desktop, while scrolling up pauses the stream and loads the existing tmux history without losing your place. Leaving the page pauses delivery; after 10 seconds away, returning starts from a fresh current screen instead of replaying stale output. If a WebSocket path is unavailable, Handmux falls back to its proven snapshot refresh automatically; Settings → Terminal refresh mode can also keep this browser on snapshots. On desktop, type into the current pane immediately without tapping “Show keyboard”; `Shift+Enter` or the draft field opens draft mode with shortcuts, upload, history, voice and multiline editing intact. `Enter` sends and stays in draft mode, `Shift+Enter` inserts a newline, and IME Enter only confirms composition; press `Esc` or click the terminal to return to direct input. Drag to select text, then copy/paste with `Cmd+C` / `Cmd+V` on macOS or `Ctrl+Shift+C` / `Ctrl+V` (or `Ctrl+Shift+V`) on Windows/Linux; `Ctrl+C` remains the terminal interrupt. Settings → Keyboard mode can auto-detect or force Mobile/Desktop for this browser only. Browser-reserved shortcuts such as `Cmd+W/T/L/R` stay with the browser.
