...
- **Built-in browser + static preview** — new tabs default to opening directly on the phone. Once `previewDomain` is configured (with routing and a certificate for `*.<previewDomain>`), any tab can instead use the computer proxy, and each tab can switch modes independently. Switching reloads the last URL Handmux knows; direct and proxy modes keep separate cookies and sign-in state. Device-local history remembers each URL's `lastMode`, while old entries follow the current default. Direct mode uses a cross-origin iframe, so sites may refuse embedding through `X-Frame-Options` or CSP, third-party cookies may be restricted, and Handmux usually cannot read the title or in-page navigation. It never silently falls back between direct and proxy: an unavailable proxy or a slow/unopenable direct page asks you to choose. Phone/desktop viewports and independent 10/30/60/120-minute or never-close timers remain available; folder preview still serves static sites.
