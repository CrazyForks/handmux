...
- **Built-in browser + static preview** — the Browser shortcut sits immediately left of Files in the top bar. Its top-right dot is blue when direct tabs are open and yellow when at least one proxy tab is open. The icon-only Home tab opens Recently Visited; one tab session keeps only its latest navigated URL. New pages default to direct mode, but the address bar’s **…** card can select direct or computer proxy before opening the URL; the address badge updates immediately, and an existing tab can still switch afterward. Once `previewDomain` is configured (with routing and a certificate for `*.<previewDomain>`), the proxy choice becomes available through the same large text segmented control. That card also contains phone/desktop view and independent 10/30/60/120-minute background-close timers. On Home, one Proxy sign-in retention picker offers Not saved (default), 1/7/30 days, or long-term encrypted storage for this device, alongside clear-all and About; on a proxy page it offers clear-this-site only. Contextual **?** buttons sit beside the corresponding action and explain its scope and parent-domain effects. Proxy pages use an orange loading bar while direct pages remain blue. Tabs, order, selection, recent visits, and timers live entirely on the current device, while the proxy worker owns only disposable runtime leases rebuilt from canonical URLs. Every proxy tab has its own Hammerhead Session, and tabs on one device share only standards-matched Cookies, allowing enterprise SSO without sharing localStorage, IndexedDB, Service Workers, tabs, or state with other devices or direct mode. Switching tabs preserves the mounted iframe; Refresh performs a real navigation. Direct mode still has cross-origin iframe limits (`X-Frame-Options`, CSP, third-party-cookie policy, unreadable titles/navigation), and Handmux never silently changes modes. Explicitly allowed loopback targets remain restricted to the exact scheme, host, and port. Folder preview still serves static sites.
