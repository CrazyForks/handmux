...
- **Built-in browser + static preview** — the Browser shortcut sits immediately left of Files in the top bar, and new tabs default to opening directly on the phone. Once `previewDomain` is configured (with routing and a certificate for `*.<previewDomain>`), any tab can instead use the computer proxy, and each tab can switch modes independently. Proxy tabs on the same device use one Cookie Profile matched against each real target's Domain, Path, Secure and other Cookie rules, so an enterprise SSO login can carry across systems; Profiles are never shared with another device, and direct mode keeps its own browser-managed cookies and state. Explicitly opened target origins use a stable short proxy host, `b-<13 lowercase base36>.<previewDomain>`; links, forms and redirects then stay in the current Hammerhead session so their native method, body and navigation semantics are preserved. Switching tabs keeps the iframe, scroll position and form state without reloading; reload happens only from the explicit Refresh control. The toolbar intentionally omits unreliable Back/Forward controls; use device-local history to reopen a previous URL. Opening a page from History adds one navigation level, so Back returns to History before a second Back minimizes Browser. Per-device encrypted persistence on the host defaults off; automatic cleanup after the device has no proxy tabs can be set to 1, 7 or 30 days (default), or never. You can clear one site's matching Cookies and proxy tabs, or all proxy login state for the device; clearing a parent-domain Cookie may also sign you out of related systems. These actions do not affect direct tabs, other devices or history. Device-local history shows each URL's last direct/proxy mode and lets you delete one record separately. Only Cookies participate in cross-system reuse—localStorage, IndexedDB and Service Workers do not. Direct mode still has cross-origin iframe limits (`X-Frame-Options`, CSP, third-party-cookie policy, unreadable titles/navigation), and Handmux never silently falls back between paths. Phone/desktop viewports and independent 10/30/60/120-minute or never-close tab timers remain available; folder preview still serves static sites.
