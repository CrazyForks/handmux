...
- 代理登录状态可由每台设备单独选择是否加密持久化到主机（默认关闭），并按 1 / 7 / 30 天或永不自动清理；支持清理单个网站或当前设备全部代理登录状态。历史记录显示最后访问模式，并可单独删除而不影响登录状态。

### Changed
- 内置浏览器的当前连接方式、页面视图、后台关闭和代理登录设置统一收进地址栏“…”菜单；连接方式使用大号文字分段按钮，新网址固定从手机直连开始，设置弹窗不再重复展示浏览器配置。顶部入口右下角会按已打开页签显示直连蓝点或代理黄点，图标本身不变色；后台页签仅提供 10 / 30 / 60 / 120 分钟关闭选项。
