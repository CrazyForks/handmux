...
### Added
- 新增顶部常驻的双模式网页预览：新页签默认由手机直连，也可在已配置 `previewDomain` 时选择经电脑代理；同一设备的代理页签共享按真实目标域规则匹配的 Cookie Profile，可在企业系统间复用 SSO 登录，不同设备与直连模式始终隔离。
...
### Changed
- 网页预览的网页缩放统一放入“…”菜单：直连和代理页面都可按 75% / 80% / 90% / 100% / 110% / 125% / 150% / 175% / 200% 逐档调整；放大时保持原网页布局宽度并显示滚动区域，网页始终可交互，标签栏和地址栏保持固定。
- 网页预览的连接方式、页面视图、后台关闭和代理登录设置统一收进地址栏“…”菜单；连接方式使用大号文字分段按钮，新页面默认手机直连，也可在输入网址前选择电脑代理，地址栏标签会即时同步，打开后仍可切换当前页签。设置弹窗不再重复展示网页预览配置；顶部入口图标会按已打开页签显示直连蓝色或代理橙色；后台页签仅提供 10 / 30 / 60 / 120 分钟关闭选项。
- 产品定位统一为“网页预览”，首次告知与“关于”明确说明它不是完整浏览器，适合开发服务、localhost、内网和允许嵌入的网站；“…”菜单新增“用系统浏览器打开”原网址。直连可能受 iframe / CSP 限制且 Handmux 未必能判断失败原因；电脑代理会转发并改写网页，也不保证兼容所有网站。未配置代理域名时引导在电脑运行 `handmux setup` 并选择“网页预览”。
- 代理网页的“…”菜单只清理本站代理 Cookie，说明问号与清理操作保持同一行；全部清理与登录持久化只出现在 Home 菜单，并移除无实际用途的“关闭网页预览”入口。
- Home 菜单新增“关于网页预览”，可随时重看首次安全告知；安全告知的确认按钮改为“好的”，不再使用“开启”表述。
- 代理网页加载进度条改用代理橙色，直连网页继续使用蓝色。
- 网页预览的标签页、顺序、当前页、历史和后台关闭计时改由每台设备本地管理；手机直连不再创建任何服务端标签，代理 worker 只维护可丢弃、可按原始网址重建的运行租约。代理进程重启不会删除设备上的标签，切换标签也不再触发服务端同步或页面重载。
- 每个代理标签使用独立的 Hammerhead Session，同一设备仅共享符合真实域规则的 Cookie Profile；关闭一个标签不会再让同站点的另一个标签失效，也不会把不同设备或直连页面混入同一运行状态。
- 网页预览替代原动态端口预览，静态目录预览继续保留。手机直连使用跨域 iframe，目标站的 `X-Frame-Options` / CSP、第三方 Cookie 或手机网络可能阻止加载，且 Handmux 无法可靠读取标题和页面内跳转；代理模式改用固定短域名 `b-<13 位小写 base36>.<previewDomain>`，只有收到 `previewDomain` 配置后才可选。Cookie Profile 仅共享 Cookie，不会跨系统共享 localStorage、IndexedDB 或 Service Worker；按网站清理父域 Cookie 时可能同时影响同一父域下的相关系统。
- 网页预览暂不展示无法可靠判断状态的前进 / 后退按钮，可从设备本地“最近访问”重新打开之前的网址；顶部最左侧入口改为无文字 Home 图标。
- 顶栏浏览器入口移到“文件”左侧；从“最近访问”进入网页时会增加一层返回栈，返回先回到最近访问页，再次返回才收起浏览器，与其他多层工具一致。

### Fixed
- 修复网页预览与其他多层界面同时存在时，返回或电脑端 `Esc` 可能绕过最上层界面的问题。
- 新建或切换到页签时，标签栏会自动滚动到当前页签；清理本站或全部代理 Cookie 只清除登录数据，不再关闭或释放已打开的页签。
- 网页预览刷新按钮改为走与地址栏导航相同的代理请求路径，不再出现按钮显示加载但网页没有真正刷新的情况。
...
- 收紧网页预览访问本机服务的边界：显式打开的 loopback 页面只可访问完全相同的 scheme、host 和 port，不能借一个已允许端口横向探测其他本机控制端口。
...
- 网页预览地址栏不再显示代理包装地址；连续打开链接时只保留最后一次请求，刷新与加载过程使用顶部进度条并可随时停止，同时阻止误触；切换页签会保留页面、滚动和表单状态，代理页也不会再因桥接脚本未响应或网站内嵌跨域 iframe 而循环刷新；网页标题会同步到页签与历史；浏览代理改由独立工作进程承载，异常退出不会带停终端主应用。
