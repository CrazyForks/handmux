...
- 终端里打印出的本机地址（`http://localhost:3000/...`、`127.0.0.1`、`0.0.0.0`、`[::1]`，带端口和路径）现在会自动识别、加下划线可点：点一下弹出小气泡「开启代理并预览」，确认后自动把该端口通过动态预览反向代理起来，并直接把 `http://localhost:3000/foo/bar` 里的 `/foo/bar` 路径打开在手机的预览浮层里（手机访问不到电脑的 localhost，代理替你转发）。同一窗口不同端口的地址各自独立起代理、可并存。没配预览域名（动态预览未开启）时，气泡会提示去设置里配置，而不是静默失败；端口没有服务在监听时也会就地说明原因。同一窗口不同端口的地址各自独立起代理、可并存（服务端并存多个反代，界面同一时刻显示最近点开的那个）。预览浮层开着时会自动续期（到期前约 1 分钟自动刷新 TTL），正在看的预览不会中途过期；收起或关掉后照常按 TTL 回收。新增前端 `web/src/localUrl.js`（回环 URL 识别，纯函数、含单测）；识别与文档路径检测按字符区间互斥，避免 `localhost:3000/foo.html` 被误当成文档路径。
