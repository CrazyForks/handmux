...
### Added
- 对话镜头里编辑文件的工具卡现在像各家 AI 编程工具那样显示**增删行数**(`+6 −5`,绿加红删),点开看**彩色 diff 明细**(逐 hunk、增/删/上下文分色),取代原先那句无聊的「已更新成功」。数据直接取自 Claude Code 转录里现成的 `toolUseResult.structuredPatch`(逐 hunk 真实 diff),不额外算;新建文件按内容行数记作 +N。
- 对话镜头改成更像普通聊天软件、可换配色：新增「对话配色」偏好（设置里选，默认**深墨**冷调暗，另有**暖白** iMessage 感亮色、**暖夜**暖调暗色），整套气泡/工具卡/门卡片/输入卡从一组 `--ct-*` token 一处换肤，终端镜头不受影响。工具活动仍**如实**展示——该运行命令就显示命令、该调工具/激活技能/调 Agent 就分别显示，点一下展开看完整命令/路径/输出；工具芯片新增覆盖技能（Skill）、Agent（Task）、搜索（Grep/Glob）、联网（WebSearch/WebFetch）、待办（TodoWrite）等类型的一句话摘要。暖白配色下把复用自终端的快捷回复彩色药丸做了对比度加深，避免白底上发灰。
