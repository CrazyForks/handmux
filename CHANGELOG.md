...
### Fixed
- 未开启设备通知时不再因等待 Service Worker / 设备 key 而误报「通知记录加载失败」；真实超时和 HTTP 错误会保留具体阶段。设置中的更新日志、通知记录、预览、目录选择器和菜单现在作为子层叠在设置页上，返回或 `Esc` 会先回到设置，再回到主屏。
- 电脑端按 `Esc` 现在只关闭最上层弹窗或浮层，连续按键可逐层返回；二级菜单、目录选择器和多级文件 / Git / 通知页面不再一次退出多层。
- Window 选中和未选中状态下，高度以及 Agent 图标与窗口名称的间距保持一致。
- 电脑触控板现在可以横向滑动超宽终端，不再被终端的纵向滚动处理拦截。
- 移动端仅在终端超宽时为横向滚动条提供独立高度，不再遮挡文字或在无需滚动时留下空白。
- 移动端终端恢复可见的纵向滚动指示，并仅在有回滚内容时为其保留独立宽度。
