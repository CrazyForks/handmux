...
### Added
- 新增顶部常驻的双模式网页预览器：新页签默认由手机直连，也可在已配置 `previewDomain` 时选择经电脑代理；同一设备的代理页签共享按真实目标域规则匹配的 Cookie Profile，可在企业系统间复用 SSO 登录，不同设备与直连模式始终隔离。
- 代理 Cookie 由每台设备单独选择是否加密持久化到电脑（默认关闭），开启后可在服务重启后恢复登录状态；支持清理单个网站或当前设备全部代理登录状态。“最近访问”显示最后访问模式，并可单独删除而不影响登录状态；同一标签会话内的页面跳转只保留最新网址。
- 电脑浏览器现在可用物理键盘直接操作终端；底部完整保留快捷用语和聊天输入框，点击输入框可先整理多行文字，Enter 发送、Shift+Enter 换行。

### Changed
- 网页预览器仅在当前标签显示关闭图标，未选中标签不再为关闭按钮预留空间；网址与静态标签按创建时间统一排列，新打开的网页标签始终追加到全部现有标签之后。成功打开的静态目录也会以绿色“静态”记录进入当前设备的“最近访问”，再次打开时按目录获取新 token，不保存旧预览地址。
- 静态目录预览融合进常驻“网页预览器”：Home 地址栏右侧可直接选择目录，绿色静态标签复用手机 / 电脑视图及网页缩放，不再单列“正在运行的预览”，也不提供系统浏览器、连接方式、Cookie、后台计时、停止或重启入口。静态租约由真实页面访问自动续期，关闭标签立即释放，异常遗留两小时后回收；重新打开设备保存的标签时会自动恢复。生命周期已与代理租约统一，不再提供 `--preview-ttl` / `HANDMUX_PREVIEW_TTL`。
- 网页预览器的网页缩放统一放入“…”菜单：直连和代理页面都可按 75% / 80% / 90% / 100% / 110% / 125% / 150% / 175% / 200% 逐档调整；放大时保持原网页布局宽度并显示滚动区域，网页始终可交互，标签栏和地址栏保持固定。
- 网页预览器顶部标签栏压缩为与文件等全屏工具接近的高度，选中标签的整体最小宽度约为四个汉字（包含关闭按钮），左侧主页键与右侧操作键保持等宽；直连、代理和静态标签统一以上边颜色条标识模式，选中后同时显示同色下边条，并移除标签标题前的小圆点；外层工具栏的代理状态图标改用柔和的暖黄色。
- 网页预览器的连接方式、页面视图、后台关闭和代理登录设置统一收进地址栏“…”菜单；连接方式使用大号文字分段按钮，新页面默认手机直连，也可在输入网址前选择电脑代理，地址栏标签会即时同步，打开后仍可切换当前页签。设置弹窗不再重复展示网页预览器配置；顶部入口图标会按已打开页签显示直连蓝色或代理暖黄色；后台页签仅提供 10 / 30 / 60 / 120 分钟关闭选项。
- 产品定位统一为“网页预览器”，首次告知与“关于”在正文说明它不是真正的浏览器，适合开发服务、localhost、内网和允许嵌入的网站；“…”菜单新增“用系统浏览器打开”原网址。直连可能受 iframe / CSP 限制且 Handmux 未必能判断失败原因；电脑代理会转发并改写网页，也不保证兼容所有网站。未配置代理域名时引导在电脑运行 `handmux setup` 并选择“网页预览器”。
- 代理网页的“…”菜单只清理本站代理 Cookie，说明问号与清理操作保持同一行；全部清理与登录持久化只出现在 Home 菜单，并移除无实际用途的“关闭网页预览器”入口。
- Home 菜单新增“关于网页预览器”，可随时重看首次安全告知；安全告知的确认按钮改为“好的”，不再使用“开启”表述。
- 代理网页加载进度条改用代理橙色，直连网页继续使用蓝色。
- 网页预览器的标签页、顺序、当前页、历史和后台关闭计时改由每台设备本地管理；手机直连不再创建任何服务端标签，代理 worker 只维护可丢弃、可按原始网址重建的运行租约。代理进程重启不会删除设备上的标签，切换标签也不再触发服务端同步或页面重载。
- 每个代理标签使用独立的 Hammerhead Session，同一设备仅共享符合真实域规则的 Cookie Profile；关闭一个标签不会再让同站点的另一个标签失效，也不会把不同设备或直连页面混入同一运行状态。
- 网页预览器替代原动态端口预览，静态目录预览继续保留。手机直连使用跨域 iframe，目标站的 `X-Frame-Options` / CSP、第三方 Cookie 或手机网络可能阻止加载，且 Handmux 无法可靠读取标题和页面内跳转；代理模式改用固定短域名 `b-<13 位小写 base36>.<previewDomain>`，只有收到 `previewDomain` 配置后才可选。Cookie Profile 仅共享 Cookie，不会跨系统共享 localStorage、IndexedDB 或 Service Worker；按网站清理父域 Cookie 时可能同时影响同一父域下的相关系统。
- 网页预览器暂不展示无法可靠判断状态的前进 / 后退按钮，可从设备本地“最近访问”重新打开之前的网址；顶部最左侧入口改为无文字 Home 图标。
...
- 最后一个代理 Session 释放后会在短暂排空在途响应后关闭对应 Hammerhead Pool 与监听端口；终端链接弹窗取消打开时也会移除临时页签并回收迟到租约。
- 修复 WebKit 中静态网页切换到电脑视图后，大尺寸缩放 iframe 的合成层可能造成页面、标签栏和菜单出现黑灰色块的问题。
- 修复网页内容已出现但进度条尚未结束时无法滚动或交互，以及切换直连、代理或静态页签时页面可能重新加载的问题；访问过的页面现在保持挂载，加载提示不再拦截网页操作。
- 修复网页预览器与其他多层界面同时存在时，返回或电脑端 `Esc` 可能绕过最上层界面的问题。
- 新建或切换到页签时，标签栏会自动滚动到当前页签；清理本站或全部代理 Cookie 只清除登录数据，不再关闭或释放已打开的页签。
- 网页预览器刷新按钮改为走与地址栏导航相同的代理请求路径，不再出现按钮显示加载但网页没有真正刷新的情况。
- 代理页签不再因 worker 异常退出、主服务重启或一次性启动地址失效而从当前设备消失；重新进入时会用设备保存的原始网址获取新的代理租约。浏览代理中被拦截或改写的 WebSocket upgrade 异常也会限制在单条连接内；worker 短暂重启或网络超时会自动恢复，不再误报为代理登录设置同步失败。
- 代理访问 `localhost` 时会在全部通过安全检查的 IPv4 / IPv6 地址间自动回退；仅监听 `127.0.0.1` 的本机服务不再因系统优先解析到 `::1` 而打不开。
- 收紧网页预览器访问本机服务的边界：显式打开的 loopback 页面只可访问完全相同的 scheme、host 和 port，不能借一个已允许端口横向探测其他本机控制端口。
...
- 网页预览器地址栏不再显示代理包装地址；连续打开链接时只保留最后一次请求，刷新与加载过程使用顶部进度条并可随时停止，网页保持可交互；切换页签会保留页面、滚动和表单状态，代理页也不会再因桥接脚本未响应或网站内嵌跨域 iframe 而循环刷新；网页标题会同步到页签与历史；浏览代理改由独立工作进程承载，异常退出不会带停终端主应用。
