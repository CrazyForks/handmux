...
### Fixed
- 键盘开着时左右横滑切换命令/聊天页不再收起键盘：起手判断「键盘是否开着」原先只看焦点在不在输入框上，而
  iOS 会在焦点已漂离输入框时仍留着键盘（切走又回来、异常失焦等），于是横滑被当成「键盘没开」而收掉——之前要
  先竖着划一下重新聚焦才能规避。现在改用键盘的真实高度判定，横滑始终把焦点带到目标页的输入框、键盘保持不动。
- 边打字边切换窗口时输入框不再弹一下又缩回去：切窗会整块重挂终端，挂载时为激活只读光标做的一次
  `term.focus()` 落在 xterm 的隐藏真实输入框上；在 iOS 上这次 focus 还处于「点标签」的手势内，会把软键盘
  瞬间重新拉起、紧接着 `blur()` 又收掉（整个界面跟着键盘上下弹）。现在键盘还开着（你正在输入）时跳过这次
  预热，光标改在你下次点终端时激活；键盘落下的常规切窗行为不变。
