...
### Added
- 手机和电脑的终端输出改为 WebSocket 实时流；首次打开即可上滑浏览最近的 tmux 历史，继续向上时沿用现有历史加载，回到底部后重新同步。离开页面超过 10 秒后会丢弃旧输出并从最新画面继续；实时推送连接失败时自动退回快照拉取，点击右上角模式标签可查看降级原因和恢复倒计时，也可在设置中让当前浏览器固定使用快照拉取。
