...
### Changed
- 内置浏览器的连接方式、页面视图、后台关闭和代理登录设置统一收进地址栏“…”菜单；连接方式使用大号文字分段按钮，新页面默认手机直连，也可在输入网址前选择电脑代理，地址栏标签会即时同步，打开后仍可切换当前页签。设置弹窗不再重复展示浏览器配置；顶部入口右上角会按已打开页签显示直连蓝点或代理黄点，图标本身不变色；后台页签仅提供 10 / 30 / 60 / 120 分钟关闭选项。
