...
- 网页预览器顶部标签栏压缩为与文件等全屏工具接近的高度，选中标签的整体最小宽度约为四个汉字（包含关闭按钮），左侧主页键与右侧操作键保持等宽；直连、代理和静态标签统一以上边颜色条标识模式，选中后同时显示同色下边条，并移除标签标题前的小圆点；外层工具栏的代理状态图标改用克制的深橙色。
- 网页预览器的连接方式、页面视图、后台关闭和代理登录设置统一收进地址栏“…”菜单；连接方式使用大号文字分段按钮，新页面默认手机直连，也可在输入网址前选择电脑代理，地址栏标签会即时同步，打开后仍可切换当前页签。设置弹窗不再重复展示网页预览器配置；顶部入口图标会按已打开页签显示直连蓝色或代理橙色；后台页签仅提供 10 / 30 / 60 / 120 分钟关闭选项。
