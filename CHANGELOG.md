...
- 代理登录状态可由每台设备单独选择是否加密持久化到主机（默认关闭），并按 1 / 7 / 30 天或永不自动清理；支持清理单个网站或当前设备全部代理登录状态。“最近访问”显示最后访问模式，并可单独删除而不影响登录状态；同一标签会话内的页面跳转只保留最新网址。
...
- 代理网页的“…”菜单只清理本站代理 Cookie，全部清理与登录持久化只出现在 Home 菜单；相关问号会说明当前设备、直连隔离与父域 Cookie 的影响，并移除无实际用途的“关闭内置浏览器”入口。
