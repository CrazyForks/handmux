...
### Fixed
- 普通授权模式下，你**回应权限弹窗后**该会话不再一直卡在「需要你」：根因是「需要你」状态没有任何关闭它的
  hook——授权普通工具（Bash/Edit 等）不触发我们监听的 PostToolUse，而手动拒绝或按 ESC 中断根本不发任何
  hook，于是收件箱从你回应那刻一直误显「需要你」，ESC 后更是永久卡住。现在服务端据会话 transcript 的写入对账：
  弹窗挂起时 Claude 阻塞、transcript 不增长；你一旦回应它就长大、修改时间越过该事件——据此判定已回应，并读
  transcript 末行区分去向：**授权（yes）或拒绝并反馈 → 恢复「进行中」**；**按 ESC 中断（末行是
  `[Request interrupted by user]`）→ 消掉状态、回落中性「在场」态**。读不到 transcript 则维持现状，绝不误消。
- 新开窗口、内容较短时不再"内容停在屏幕中间、光标却在最底"，要打一个字才对齐：根因是首帧按 xterm 默认行数
  （24）算的底部留白，紧接着 fit 把网格长大到填满屏幕却没重算留白，于是短内容被留白顶在中部、光标按长大后的
  网格底摆放。现在 fit 定稿后按最终行数重算留白重绘（等修正帧落地再显示，无闪烁），短内容一开窗就贴底与光标吻合。
