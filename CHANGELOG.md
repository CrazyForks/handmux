...
### Fixed
- 对话视图：/clear 后旧会话内容残留在屏上（切走再回来才正常）；/clear 触发后视图也可能停在空白不再更新（新增 SessionStart hook 重新绑定）。
- 对话视图：发送 /compact 当时无任何显示（命令脚手架要等压缩完成才落盘），现在发送即回显命令 pill，压缩完成后自动替换为真实标记与结果。
- 对话视图：点完问题卡的最终确认后，偶发闪出一个「允许/拒绝」备用卡片、约一秒后自动消失。
- 对话视图：问题卡的遮罩把顶部标签栏与功能键也盖住了（现在只遮视图所在的下半区）；过短的确认卡盖不住输入区的快捷键。
- 对话视图：工具详情弹窗在输入内容很长时，输出结果怎么滑都滑不到（容器 flex 收缩顶替了溢出滚动）；长命令现在封顶自带滚动。
- 对话视图：消息都很短、整屏装不满时无法上拉加载更早的消息，现在会自动补拉上一页直到装满或没有更早内容。
