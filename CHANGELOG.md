...
### Added
- 新增顶部常驻的双模式内置浏览器：新页签默认由手机直连，也可在已配置 `previewDomain` 时选择经电脑代理；同一设备的代理页签共享按真实目标域规则匹配的 Cookie Profile，可在企业系统间复用 SSO 登录，不同设备与直连模式始终隔离。
...
### Changed
- 内置浏览器的网页缩放统一放入“…”菜单：直连和代理页面都可按 75% / 80% / 90% / 100% / 110% / 125% / 150% / 175% / 200% 逐档调整；放大时保持原网页布局宽度并显示滚动区域，网页始终可交互，标签栏和地址栏保持固定。
- 内置浏览器的连接方式、页面视图、后台关闭和代理登录设置统一收进地址栏“…”菜单；连接方式使用大号文字分段按钮，新页面默认手机直连，也可在输入网址前选择电脑代理，地址栏标签会即时同步，打开后仍可切换当前页签。设置弹窗不再重复展示浏览器配置；顶部入口图标会按已打开页签显示直连蓝色或代理橙色；后台页签仅提供 10 / 30 / 60 / 120 分钟关闭选项。
- 内置浏览器会在连接方式旁提示代理访问较慢且不保证兼容所有网站；未配置预览域名时明确引导在电脑运行 `handmux setup` 并选择“内置浏览器”，setup 菜单与说明也统一使用该名称。首次告知与“关于”中同时说明直连 iframe/CSP 限制和代理兼容边界。
- 代理网页的“…”菜单只清理本站代理 Cookie，说明问号与清理操作保持同一行；全部清理与登录持久化只出现在 Home 菜单，并移除无实际用途的“关闭内置浏览器”入口。
- Home 菜单新增“关于内置浏览器”，可随时重看首次安全告知；安全告知的确认按钮改为“好的”，不再使用“开启”表述。
- 代理网页加载进度条改用代理橙色，直连网页继续使用蓝色。
- 内置浏览器的标签页、顺序、当前页、历史和后台关闭计时改由每台设备本地管理；手机直连不再创建任何服务端标签，代理 worker 只维护可丢弃、可按原始网址重建的运行租约。代理进程重启不会删除设备上的标签，切换标签也不再触发服务端同步或页面重载。
...
- 内置浏览器暂不展示无法可靠判断状态的前进 / 后退按钮，可从设备本地“最近访问”重新打开之前的网址；顶部最左侧入口改为无文字 Home 图标。
- 顶栏浏览器入口移到“文件”左侧；从“最近访问”进入网页时会增加一层返回栈，返回先回到最近访问页，再次返回才收起浏览器，与其他多层工具一致。

### Fixed
- 内置浏览器不再让长时间无法完成的网页一直显示加载：5 秒后会按手机直连或电脑代理说明可能原因，并提供继续等待、重试及手动切换连接方式。
- 修复内置浏览器与其他多层界面同时存在时，返回或电脑端 `Esc` 可能绕过最上层界面的问题。
- 新建或切换到页签时，标签栏会自动滚动到当前页签；清理本站或全部代理 Cookie 只清除登录数据，不再关闭或释放已打开的页签。
- 内置浏览器刷新按钮改为走与地址栏导航相同的代理请求路径，不再出现按钮显示加载但网页没有真正刷新的情况。
...
- 收紧内置浏览器访问本机服务的边界：显式打开的 loopback 页面只可访问完全相同的 scheme、host 和 port，不能借一个已允许端口横向探测其他本机控制端口。
...
- 内置浏览器地址栏不再显示代理包装地址；连续打开链接时只保留最后一次请求，刷新与加载过程使用顶部进度条并可随时停止，同时阻止误触；切换页签会保留页面、滚动和表单状态，代理页也不会再因桥接脚本未响应或网站内嵌跨域 iframe 而循环刷新；网页标题会同步到页签与历史；浏览代理改由独立工作进程承载，异常退出不会带停终端主应用。
