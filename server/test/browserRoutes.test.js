...
describe('browser routes', () => {
  it('creates a tab using the current forwarded Handmux origin', async () => {
    ...
    const res = await request(appFor(browser))
      .post('/browser-tabs')
    ...
    expect(browser.create).toHaveBeenCalledWith({
      url: 'https://target.example/path', origin: 'https://internal.example:30443', closeAfterMinutes: 10,
    });
  ...
  it.each([10, 30, 60, 120, null])('accepts closeAfterMinutes=%s', async (closeAfterMinutes) => {
    const browser = browserFake();
    const res = await request(appFor(browser)).post('/browser-tabs').send({ url: 'https://target.example/', closeAfterMinutes });
    expect(res.status).toBe(201);
  });
  ...
  it.each([undefined, 0, 5, 15, 180, '10'])('rejects unsupported closeAfterMinutes=%s', async (closeAfterMinutes) => {
    ...
    const res = await request(appFor(browser)).post('/browser-tabs').send(body);
  ...
  it.each(['ftp://target.example/', 'javascript:alert(1)', 'not a URL'])('rejects unsupported target %s', async (url) => {
    const browser = browserFake();
    const res = await request(appFor(browser)).post('/browser-tabs').send({ url, closeAfterMinutes: 10 });
    expect(res.status).toBe(400);
  ...
  it('lists tabs', async () => {
    const browser = browserFake();
    const res = await request(appFor(browser)).get('/browser-tabs');
  ...
  it('updates one tab visibility and its independent close duration', async () => {
    const browser = browserFake();
    const res = await request(appFor(browser)).patch('/browser-tabs/tab-a/visibility').send({ visible: false, closeAfterMinutes: 30 });
    expect(res.status).toBe(200);
    expect(browser.setVisible).toHaveBeenCalledWith('tab-a', false, 30);
  });

  it('navigates an existing tab', async () => {
    const browser = browserFake();
    const res = await request(appFor(browser)).post('/browser-tabs/tab-a/navigate').send({ url: 'https://next.example/' });
    expect(res.status).toBe(200);
    expect(browser.navigate).toHaveBeenCalledWith('tab-a', 'https://next.example/');
  });

  it('returns 404 when updating a missing tab', async () => {
    const browser = browserFake();
    browser.setVisible.mockReturnValue(null);
    const res = await request(appFor(browser)).patch('/browser-tabs/missing/visibility').send({ visible: false, closeAfterMinutes: 10 });
    expect(res.status).toBe(404);
  });

  it('closes one tab', async () => {
    const browser = browserFake();
    const res = await request(appFor(browser)).delete('/browser-tabs/tab-a');
    expect(res.status).toBe(204);
    expect(browser.closeTab).toHaveBeenCalledWith('tab-a');
  });
});
