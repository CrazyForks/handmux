...
export default {
  ...
  'shortcuts.edit': '编辑',
  'shortcuts.move': '移动位置',
  'shortcuts.addPositionPrompt': '放到哪里',
  'shortcuts.movePrompt': '选择新位置（当前：第 {n} 位）',
  'shortcuts.positionFirst': '最前（第 1 位）',
  'shortcuts.positionAfter': '放在 {item} 后（第 {n} 位）',
  'shortcuts.positionLast': '最后（第 {n} 位）',
  'shortcuts.delete': '删除',
  ...
  'setup.secPort': '端口',
  'setup.secToken': '令牌',
  'setup.secBrowser': '内置浏览器',
  'setup.browserOff': '未配置 · 仅手机直连',
  'setup.askBrowserDomain': '内置浏览器预览域名（留空 = 仅手机直连）',
  'setup.browserAbout': '配置后，内置浏览器可通过电脑网络访问内网、localhost 和普通网站；不配置时仍可使用手机直连。',
  'setup.browserRoute': '只填预览域名（如 preview.example.com）。请将 *.preview.example.com 路由到 Handmux 网关。',
  'setup.browserHttps': '预览域名需要 HTTPS 和通配证书；保存并重启后，“经电脑代理”才可用。',
  'setup.browserTlsCf': '  TLS：Cloudflare 免费证书通常只覆盖一级通配域名；更深层级可能需要 Advanced Certificate Manager。',
  'setup.browserTlsEdge': '  TLS：请在自己的边缘配置通配证书（例如 Let’s Encrypt 的 *.preview.your.domain）。',
