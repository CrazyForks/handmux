...
export default {
  ...
  'restore.selectionCancelled': '没有选择备份。准备好后可再次运行 `handmux restore`。',
  ...
  'restore.dryRunHint': '运行 `handmux restore` 继续。',
  ...
  'restore.retrySession': '只重试这个会话: handmux restore --checkpoint {checkpoint} --session {session}',
  'restore.resultSummary': '已恢复 {restored} 个会话；{already} 个已存在；{failed} 个失败。',
  'restore.error': '备份 {checkpoint} 在恢复命令阶段失败: {error}',
  'restore.retry': '修复上面报告的原因后，运行 `handmux restore --checkpoint {checkpoint}` 重试。',
